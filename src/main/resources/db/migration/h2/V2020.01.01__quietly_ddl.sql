...
create TABLE QTL_STEP_DEFINITION (
  ...
  api_headers varchar NULL COMMENT '调用接口行为才用得到。固定JSON格式(其中的值可以使用占位符，以 ${} 作为占位符来引用上一个执行步骤传递过来的参数。)：{"token": "${token}"}',
  api_body_args varchar NULL COMMENT '调用接口行为才用得到。固定JSON格式(其中的值可以使用占位符，以 ${} 作为占位符来引用上一个执行步骤传递过来的参数。)：{"username": "jack", "userId": "${userId}"}',
...
insert into QTL_STEP_DEFINITION values(1, 1, 'ACTION_CALL_API', null, null, 1, null, null, null, null, 1, 'CREATED', 0, '{"url":"http://localhost:8080/step-definition/getStepList", "headers": {"Content-Type": "application/json;charset=UTF-8"}, "requestBody": {}}', null, 'admin', CURRENT_TIMESTAMP(), 'admin', CURRENT_TIMESTAMP());
insert into QTL_STEP_DEFINITION values(2, 1, 'ASSERT_API_RESPONSE_HTTP_CODE', null, null, null, null, null, '200', null, 2, 'CREATED', 0, null, null, 'admin', CURRENT_TIMESTAMP(), 'admin', CURRENT_TIMESTAMP());
insert into QTL_STEP_DEFINITION values(3, 1, 'ASSERT_API_RESPONSE_BY_JSON_PATH', '$[0].key', null, null, null, null, 'ACTION_CALL_API', null, 3, 'CREATED', 0, null, null, 'admin', CURRENT_TIMESTAMP(), 'admin', CURRENT_TIMESTAMP());
insert into QTL_STEP_DEFINITION values(4, 1, 'ASSERT_API_RESPONSE_BY_JSON_PATH', '$[0].name', null, null, null, null, '动作-调用接口', null, 4, 'CREATED', 0, null, null, 'admin', CURRENT_TIMESTAMP(), 'admin', CURRENT_TIMESTAMP());
insert into QTL_STEP_DEFINITION values(5, 1, 'ASSERT_DB_BY_QUERY_SQL', 'SELECT http_method FROM QTL_API_DEFINITION where id=1;', null, null, null, null, 'GET', null, 5, 'CREATED', 0, null, null, 'admin', CURRENT_TIMESTAMP(), 'admin', CURRENT_TIMESTAMP());
insert into QTL_STEP_DEFINITION values(6, 1, 'ASSERT_DB_BY_QUERY_SQL', 'SELECT project_id, http_method FROM QTL_API_DEFINITION where id=1;', null, null, null, null, '{"project_id": 1, "HTTP_METHOD": "GET"}', null, 6, 'CREATED', 0, null, null, 'admin', CURRENT_TIMESTAMP(), 'admin', CURRENT_TIMESTAMP());
insert into QTL_STEP_DEFINITION values(7, 1, 'ASSERT_DB_BY_QUERY_SQL', 'SELECT project_id, http_method FROM QTL_API_DEFINITION where id=1;', null, null, null, null, '[{"project_id": 1, "http_method": "GET"}]', null, 7, 'CREATED', 0, null, null, 'admin', CURRENT_TIMESTAMP(), 'admin', CURRENT_TIMESTAMP());

...
create TABLE QTL_DICT_DATA (
  ...
  update_time datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON update CURRENT_TIMESTAMP COMMENT 'Revisor time',
  PRIMARY KEY (id),
  UNIQUE KEY QTL_DICT_DATA_DICTTYPEID_NAME (dict_type_id, name) USING BTREE,
...
create TABLE QTL_STEP_DEFINITION (
  ...
  field_1 varchar NULL COMMENT '备用字段。如果用户需要自定义 Step 定义的话，上面这些字段可能不够，所以留几个备用字段。下同。',
  field_2 varchar NULL COMMENT '备用字段。如果用户需要自定义 Step 定义的话，上面这些字段可能不够，所以留几个备用字段。下同。',
  field_3 varchar NULL COMMENT '备用字段。如果用户需要自定义 Step 定义的话，上面这些字段可能不够，所以留几个备用字段。下同。',
  field_4 varchar NULL COMMENT '备用字段。如果用户需要自定义 Step 定义的话，上面这些字段可能不够，所以留几个备用字段。下同。',
  field_5 varchar NULL COMMENT '备用字段。如果用户需要自定义 Step 定义的话，上面这些字段可能不够，所以留几个备用字段。下同。',
  field_6 varchar NULL COMMENT '备用字段。如果用户需要自定义 Step 定义的话，上面这些字段可能不够，所以留几个备用字段。下同。',

...
public enum StepType implements IEnum<String> {
     ...
     */
    ACTION_EXECUTE_UPDATE_SQL("执行更新 SQL 语句"),

    /**
     * 行为：执行一条 select SQL 语句
     */
    ACTION_EXECUTE_QUERY_SQL("执行查询 SQL 语句"),
     ...
     */
    ASSERT_JSON_OBJECT("JSON-OBJECT"),

    /**
     * 断言：上一个Action执行后，对其返回的结果进行JSON断言。断言期待的JSON值和 response 中响应的实际JSON值是否相匹配。
     */
    ASSERT_JSON_ARRAY("JSON-ARRAY"),
